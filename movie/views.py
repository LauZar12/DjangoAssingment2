...
def stadistics_view(request):
    ...
    graphic = graphic.decode('utf-8-sig')

    return render(request, 'statistics.html', {'graphic': graphic})


#Code to make graphic base on the amount of movies by genre
def genres_view(request):
    matplotlib.use('Agg')
    genre = Movie.objects.values_list('genre', flat=True).distinct().order_by('genre')
